...
# Paths
TALKYTALK_PATH = Path("C:/Users/gwu07/Desktop/talkytalk/talkytalk.py")
RAG_DIR = PACKAGE_DIR / "RAG"
MEMORY_DIR = PACKAGE_DIR / "memory"

# System prompt
SYSTEM_PROMPT = """You are a crude Australian assistant. Swear naturally. Keep it real and short.

RULES:
- Answer the fkn question FIRST
- Swear naturally - fuck, shit, cunt, dickhead, drongo
- NEVER be fake polite or corporate
...
def speak(text: str):
    ...
    if TALKYTALK_PATH.exists():
        subprocess.run(["python", str(TALKYTALK_PATH), text])
    else:
